...
;
    ...
    async getPasswordRegex() {
        ...
        this.passwordRegex = (options.consecutive ? '' : '(?!.*(.)\\1{1})')
      + '(?=(.*[\\d]){' + options.number + ',})(?=(.*[a-z]){'
      + options.lowercase
      + ',})(?=(.*[A-Z]){'
      + options.uppercase
      + ',})(?=(.*[@#$%!]){'
    ...
    async resetPassword() {
        ...
        if (this._email.validity.valid) {
            ...
            if (res.status === 200) {
                this.showLogin();
                this.showSuccess("Thanks! If there's an account associated with this email, we'll send the password reset instructions immediately.");
                this._email.value = '';
            } else {
    ...
    getRootAdapterMethods() {
        return Object.assign({
          ...
          registerValidationAttributeChangeHandler: handler => {
            ...
            const observer = new MutationObserver(mutationsList => handler(getAttributesList(mutationsList).filter(e=>e!='pattern')));
            const config = {
              attributes: true
            };
