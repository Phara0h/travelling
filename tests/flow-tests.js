...
module.exports = () => {
    ...
    describe('Test Multi User Group Permissions', () => {
      ...
      test('User 2 Inherit from Group 3', async () =>{
        var res = await Travelling.Group.Type.User.addGroupInheritance('testgroup', 'test2', 'group3', 'group', userContainer.user2Token);
        expect(res.statusCode).toEqual(200);
      });
      ...
      test('User 2 Remove From Group 1', async () =>{
        var res = await Travelling.Group.User.removeGroupInheritance('group3', 'test2group', 'test2', 'group1', 'testgroup', userContainer.user2Token);
        expect(res.statusCode).toEqual(200);
        //console.log(res.body)
      });
