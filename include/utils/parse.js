...
module.exports = {
  getIp: function getIp(req) {
    if (config.misc.cloudflareIP) {
      return req.headers['cf-connecting-ip'] || req.headers['CF-Connecting-IP'] || req.headers['CF-CONNECTING-IP'] || req.headers['x-forwarded-for'] || req.headers['X-Forwarded-For'] || req.headers['x-forwarded-host'] || req.headers['X-Forwarded-Host'] || req.ip;
    }
