    ...
    return new Promise((resolve, reject) => {
      crypto.randomBytes(32, async (err, secret) => {
        ...
        await TokenStore.set(
          user_id,
          type,
          (await this._hashToken(user_id, secret.toString('base64'))) + '_' + user_id,
          expiration
        );
  ...
  static getOAuthCode(user_id, client_id, redirectURI) {
    return new Promise(async (resolve, reject) => {
      ...
      for (var i = 0; i < token.urls.length; i++) {
        ...
        if (
          tokenUrlPath.length == redirectUrlPath.length ||
          (tokenUrlPath.length <= redirectUrlPath.length + 1 &&
            (tokenUrlPath[tokenUrlPath.length - 1] == '*' || tokenUrlPath[0] == '*'))
        ) {
          found = true;
          break;
        }
